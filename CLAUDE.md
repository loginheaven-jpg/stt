...
**모델 생성만으로는 CUDA가 되는지 알 수 없다.** 실측이다 — 생성 직후 CUDA 모듈 0개, 실제 음성을 인코딩해야 20개가 적재된다. 무음이나 잡음은 인코더를 돌리지 않아 탐침이 되지 못한다.

### 대체 경로 — 전사 루프까지 감싼다

그래서 `try`를 모델 생성이 아니라 **한 판 전체**에 건다. `one_pass(device, compute)`가 모델 적재부터 구간 루프까지를 돌고, CUDA 계열 오류면 CPU로 **처음부터** 다시 한다.
