...
| 정답지 | `sample3.txt` | 없음 |
| 기준값 | 오귀속 0% · 정답률 46.67% · CER 11.46% · 간투사 44.44% · 고유명사 100% · 삽입 0 · 화자 2 | 화자 2 · 발화 143 · 어절 1358 · 교정 1반영 0보류 · 종료 1087초 · 배속 22.73× |
...
### GPU 메모리를 돌려주는 자리 — `free_gpu_cache()`

**참조를 놓는 것만으로는 GPU 메모리가 드라이버로 돌아가지 않는다.** torch의 할당기가
해제한 블록을 자기 풀에 쥔다. 그런데 **whisper는 `ctranslate2`의 별도 할당기를 쓴다.**
그래서 pyannote가 한 번 돌아 torch 풀이 부풀면 다음 항목의 전사가 쓸 메모리가 없다.

부르는 곳이 둘이다.

| 곳 | 왜 |
|---|---|
| `run_diarization()`의 `finally` | **여기가 핵심이다.** 다음 항목의 전사가 굶지 않게 |
| `release_cache()` | 앱을 띄워 둔 채로 다른 프로그램이 GPU를 쓸 수 있게 |

실측이다 (RTX 3060 12GB · 2026-08-15). 고치기 전 —

```
시작 전                              GPU    593 MiB
1번 sample3.wav (26초)   총  30.7초 · GPU 11938 MiB   ← 화자 분리가 부풀린다
2번 이승은코치 (18분)     전사 12.32x · 총 175.2초
3번 이승은코치 (18분)     전사 14.98x · 총 160.1초
```

같은 음원을 **혼자** 돌리면 전사가 **35.9×**다. 항목 안 순서가 전사 → 화자 분리라
첫 항목만 메모리가 넉넉하고 둘째부터 셋 중 하나로 느려진다. 대기열이 이 도구의
핵심 기능이므로 **실사용에서 늘 걸리는 길이었다** — 2026-08-15에 2시간 13분짜리가
11.34×로 돈 것이 이것이다.

**실패 경로에서도 돌려준다.** 실패했다고 쥐고 있을 이유가 없다.

이 자리를 지키는 것은 `tests/queue_test.py`의 연속 항목 절과 R2 배속이다. **한 건만
돌려서는 절대 드러나지 않는다** — 회귀를 한 건으로 줄이지 않는다.
