...
| 1 | 대기열 · 이력 · 설정 저장 · 모델 캐시 (백엔드) | **끝났다** |
| 2 | 화면 재구성 | 다음 |
| 3 | 무창 실행 · 종료 버튼 · 진단 화면 | — |
| 4 | 설치 프로그램 · 설치 문서 | — |

대기열은 백엔드까지 들어갔다. 여러 건을 걸어두면 위에서부터 하나씩 돌고, 앱을 껐다 켜도 남은 항목이 살아 있다. 한 건이 실패해도 다음이 계속된다. 같은 설정이 이어지면 모델을 다시 올리지 않는다.

**화면은 아직 따라오지 않았다.** 지금 화면에서는 파일을 하나씩 고른다. 대기열을 눈으로 다루려면 2단계가 필요하다.
