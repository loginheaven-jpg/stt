...
기록은 `data/app.log`에 남는다. 창 없이 띄웠을 때도 실패 사유가 여기 남는다. 화면의 **진단**을 누르면 파이썬·패키지·토큰·모델·폴더 상태와 최근 기록 200줄을 한자리에서 볼 수 있다.
...
| 구역 | 하는 일 |
|---|---|
| 지금 하는 일 | 배속·남은 시간·시간 눈금 막대·받아쓰는 내용. 현재 중단, 전체 중지 |
| 대기열 | 남은 항목. 순서 이동, 빼기. 중단된 작업은 여기서 다시 건다 |
